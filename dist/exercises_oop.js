...
class Livro {
    ...
    _rate = 0;
    static _allLogged = 0;
    constructor(isbn, title, author, _totalPages) {
        this.isbn = isbn;
        this.title = title;
        this.author = author;
    ...
    rate(grade) {
        if (grade < 0 || grade > 5) {
            throw new Error('Grades are on a range from 0-5');
        }
        this._rate = grade;
    }
    static getAllLogged() {
        return Livro._allLogged;
    }
}
// Teste:
const livro1 = new Livro('978-3-16-148410-0', 'Clean Code', 'Robert Martin', 431);
const livro2 = new Livro('978-0-13-468599-1', 'TypeScript Handbook', 'Microsoft', 230);
...
//
// livro1.avaliar(6); // ❌ Error: Avaliação deve ser entre 0 e 5
//
// // livro1.isbn = "outro-isbn"; // ❌ Erro: readonly
// // livro1._avaliacao = 3;      // ❌ Erro: private
// ============================================================
// EXERCÍCIO 02 - Getters e Setters com validação de tipo
// Dificuldade: ⭐⭐
// ============================================================
